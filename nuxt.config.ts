...
export default defineNuxtConfig({
  ...
  app: {
    baseURL: process.env.NODE_ENV === 'production' ? '/REPO_NAME/' : '/', // GitHub Pages用のbaseURLを追加
    head: {
...
function defineNuxtConfig(config: {
  ...
  app: {
    ...
    head: {
      ...
      meta: ({ charset: string; } | { name: string; content: string; } | { hid: string; property: string; content: string; })[];
